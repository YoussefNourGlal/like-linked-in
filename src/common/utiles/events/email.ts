...
Emailevent.on("ApplicationStatus", async (data: EmailEvent) => {
  try {
    data.subject = "Application status";
    data.text = `Hello ${data.username}, your application is ${data.status},your email is ${data.to}`;
